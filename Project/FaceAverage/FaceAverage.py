...
def similarityTransform(inPoints, outPoints):
    ...
    x_out = c60*(outPts[0][0] - outPts[1][0]) - s60 * \
        (outPts[0][1] - outPts[1][1]) + outPts[1][0]
    y_out = s60*(outPts[0][0] - outPts[1][0]) + c60 * \
        (outPts[0][1] - outPts[1][1]) + outPts[1][1]

    outPts.append([np.int(x_out), np.int(y_out)])

    t_form = cv2.estimateAffinePartial2D(np.array([inPts]), np.array([outPts]))
...
if __name__ == '__main__':
    ...
    boundaryPts = np.array([(0, 0), (w/2, 0), (w-1, 0),
    ...
    for i in range(0, numImages):
        ...
        t_form = similarityTransform(eyecornerSrc, eyecornerDst)

        # Apply similarity transformation
        img = cv2.warpAffine(images[i], t_form, (w, h))

        # Apply similarity transform on points
        points2 = np.reshape(np.array(points1), (68, 1, 2))

        points = cv2.transform(points2, t_form)
